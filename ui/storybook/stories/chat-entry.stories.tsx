...
const meta = {
  ...
  parameters: { layout: "fullscreen", docs: { description: { component:
    "Review-only proposal: Chats contains starred and recent conversations, with the first-created agent always present as a default entry. First use uses the same compact rows as returning chats. The aligned compose icon appears on hover or keyboard focus (and stays visible on touch). The picker shows search and agent results with roles and paused status. The layout, transcript, and composer are production components. Picker, sidebar, landing, and pause explanation are Storybook-only. Selecting and sending use in-memory fixtures; no real agents run."
  } } },
...
export const SearchByRole: Story = { name: "04 · Find someone by role", args: { entryScenario: "search" } };
export const NoResults: Story = { name: "05 · No matches · Recover search", args: { entryScenario: "no-results" } };
export const PausedAgent: Story = { name: "06 · Paused agent · Clear next step", args: { entryScenario: "paused" } };
